...
if(V){
 ...
 test('binary file parser round trips and rejects invalid envelopes',async()=>{const k=V.generateKey(),e=await V.seal(text,k);assert.deepEqual(V.parseFile(e),e);for(const b of [new Uint8Array(0),e.slice(0,20),new Uint8Array(4*1024*1024+1)])assert.throws(()=>V.parseFile(b));const bad=e.slice();bad[7]=50;assert.throws(()=>V.parseFile(bad));});
 test('legacy HTML imports only the data and rejects duplicate payloads',async()=>{const e=await V.seal(text,V.generateKey());const html='<script id="vault-payload" type="application/octet-stream">'+Buffer.from(e).toString('base64')+'</script><script>throw new Error("must never execute")</script>';assert.deepEqual(V.parseFile(new TextEncoder().encode(html)),e);assert.throws(()=>V.parseFile(new TextEncoder().encode(html+html)));assert.throws(()=>V.parseFile(new TextEncoder().encode('<html>invalid</html>')));});
}
